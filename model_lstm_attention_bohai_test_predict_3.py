...
f = open('results_test_compare_3.txt', 'a')
save_folder = 'saved_model_bohai_test_compare_3'
...
for filename in filenames:
    ...
    x = Y['data_norm']
    dataset = x
    #dataset = np.reshape(data_sst, (data_sst.shape[2], data_sst.shape[0], data_sst.shape[1]))
    ## 整数编码
    ...
    OUTPUT_DIM = 3
    evaluate_X, evaluate_Y = create_dataset_test(test, TIMESTEPS, OUTPUT_DIM)
    input_data = test_X
    predictions = input_data[:, (input_data.shape[1] - OUTPUT_DIM):input_data.shape[1], :, :, :]
    for t in range(3):
        prediction = model.predict(input_data)
        predictions[:, t:t + 1, :, :, :] = prediction
        input_data[:, 0:6, :, :, :] = input_data[:, 1:7, :, :, :]
        input_data[:, 6:7, :, :, :] = prediction
    y_hat = predictions

    # pretrained_model = load_model(save_folder + '/' + new_filename + '.h5', compile=False)
    # pretrained_model.compile(loss=vae_loss, optimizer= tf.optimizers.Adam(lr=0.001,decay=9))
    # y_hat = pretrained_model.predict(test_X)

    inv_yhat = y_hat[7:2907, :, :, :, 0]
    inv_testY = evaluate_Y[0:2900, :, :, :, 0]
